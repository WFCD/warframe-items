...
class Parser {
  ...
  parse(data) {
    ...
    data.api.every((chunk) => {
      if (chunk.category === 'Recipes') return true;
      result.push({
  ...
  addComponents(item, category, blueprints, data, secondPass) {
    ...
    blueprint.ingredients.every((ingredient) => {
      let component;
      data.api.every((dataCategory) => {
        component = dataCategory.data.find((i) => i.uniqueName === ingredient.ItemType);
        return !component;
      });
      if (!component) {
  ...
  addType(item) {
    ...
    require('../config/itemTypes.json').forEach((type) => {
  ...
  addTradable(item) {
    ...
    const tradableRegex =
      /(Prime|Vandal|Wraith\w|\wWraith|Rakta|Synoid|Sancti|Vaykor|Telos|Secura|Ayatan|Prisma)(?!Derelict)/i;
  ...
  addResistanceData(item, category) {
    ...
    const parseAffectors = (affectors) => affectors.split(' ').map((element) => {
        if (element.includes('+')) {
          // positives
    ...
    const parseArmor = (enemy) => enemy.resistValues.map((resist, index) => ({
        amount: resist,
