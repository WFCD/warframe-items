...
class Build {
  async init() {
    ...
    let warningNum = 0;
    Object.keys(parsed.warnings).forEach(warning => {
  ...
  applyCustomCategories(data) {
    const result = {};
    data.every((chunk) => {
      if (chunk.category === 'Recipes') return true; // Skip blueprints
  ...
  saveJson(categories, i18n) {
    ...
    categories.forEach(category => {
      ...
      all.push(...data);
      // this is stringified, parsed, then stringified again to minify
      fs.writeFileSync(
  ...
  async saveImages(items, manifest) {
    ...
    await Promise.all(items.map(async (item) => {
      ...
      if (item.components) {
        await Promise.all(item.components.map(async (component) => this.saveImage(component, true, duplicates, manifest)));
      }
      bar.tick();
    }));
